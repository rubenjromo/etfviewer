...
def parse_portfolio(text):
    ...
    for line in text.strip().splitlines():
        if ":" in line:
            # Línea 47 corregida: usando espacio normal
            t, w = line.split(":", 1)
            try:
...
# Helpers
# --------------------------
# La caché de Streamlit ayuda a que el script sea más rápido en el servidor
...
if st.button("Calcular"):
    with st.spinner("Cargando información..."):
        ...
        # ETF Metrics
        # --------------------------
        st.markdown("### Métricas de ETFs individuales")
        ...
        fig_pie = px.pie(names=labels, values=values, hole=0.3, title="Top 10 Holdings")
        ...
        fig_inc = px.pie(eif, names="Ticker", values="Income", hole=0.3, title="Distribución de dividendos por ETF")
        ...
        fig_growth = px.line(normalized, title="Crecimiento Acumulado 5 Años")
        ...
        corr = returns.corr()
        fig_corr = px.imshow(corr, text_auto=".2f", color_continuous_scale="RdYlGn", title="Matriz de Correlación")
        ...
        fig_vol = px.line(rolling, title="Volatilidad Anualizada (252 días)")
